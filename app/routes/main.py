...
def request_leave():
    if request.method == 'POST':
        employee = Employee.query.get(session['user_id'])
        leave_type = request.form.get('leave_type') or request.form.get('type')
        ...
        if leave_type in ['annual', '반차(오전)', '반차(오후)', 'half_day']:
            if is_holiday(start_date) or is_holiday(end_date):
                flash('주말 및 공휴일에는 연차를 사용할 수 없습니다.')
                return redirect(url_for('main.request_leave'))

        if leave_type in ['반차(오전)', '반차(오후)', 'half_day'] and start_date != end_date:
            flash('반차는 하루만 신청 가능합니다.')
            return redirect(url_for('main.request_leave'))
            
        days = count_weekdays(start_date, end_date)
        if leave_type in ['반차(오전)', '반차(오후)', 'half_day']:
            days = 0.5
        ...
        for req in existing_requests:
            if leave_type in ['반차(오전)', '반차(오후)']:
                if req.leave_type.startswith('반차'):
                    if req.start_date == start_date and req.leave_type != leave_type:
                        continue
                conflict = True
                break
            else:
